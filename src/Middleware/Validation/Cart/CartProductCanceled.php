    ...
    public function __invoke(Request $request, RequestHandler $handler): Response
    {
        ...
        try {
            ...
            if (!$check_permission_admin) {
                if ( ! ( ($cart_product['user_id'] == $session->user_id) || ($cart_product['product']['user_id'] == $session->user_id) ) ) {
                    $response = new Response();
                    return $this->response($response, 422, [
                        'errors' => [
            ...
            if (!$states_allowed[$cart_product['state']]) {
                $response = new Response();
                return $this->response($response, 422, [
                    'errors' => [
                        'cart_product_id' => [
                            'State product must be: ' . implode(', ', $states_allowed ),
